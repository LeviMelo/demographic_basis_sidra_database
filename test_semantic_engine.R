...
# =======================================================================
# --- SECTION 2: SEMANTIC INDEX ENGINE (REDESIGNED) ---
# =======================================================================

#' Builds a multi-level semantic database from the raw metadata.
#'
#' This new version creates separate, searchable semantic indexes for
#' Table Titles, Variables, and Classifications.
#'
build_semantic_database <- function(raw_db_path = RAW_DB_PATH,
                                    semantic_db_path = SEMANTIC_DB_PATH,
                                    model_name = "paraphrase-multilingual-mpnet-base-v2") {
  
  message("--- Entering BUILD mode for Multi-Level Semantic Database (V8.0) ---")
  ...
  # --- Create a container for the multiple embedding matrices ---
  ...
  if(nrow(corpus_class) > 0) {
    ...
    # Create a unique ID for each classification instance to use as rownames
  ...
  semantic_database <- list(
    db = db_raw,
    embeddings = embeddings_list # The list containing multiple embedding matrices
  )
  
  message(paste("Saving new component-level semantic database to:", semantic_db_path))
  saveRDS(semantic_database, file = semantic_db_path)
  message("--- MULTI-LEVEL SEMANTIC DATABASE BUILD COMPLETE ---")
  return(invisible(semantic_database))
}

# =======================================================================
# --- SECTION 3: UNIFIED QUERY ENGINE (COMPLETELY REWRITTEN) ---
# =======================================================================
...
query_sidra <- function(mode = "hybrid",
  ...
  # --- Load Data ---
  if (!file.exists(semantic_db_path)) stop("Semantic DB not found.")
  semantic_db <- readRDS(semantic_db_path)
  db <- semantic_db$db
  
  # --- Helper: Semantic Search (Corrected with type conversion) ---
  ...
  # --- Helper: Relational Filtering (Complete, not a placeholder) ---
  .apply_relational_filters <- function(candidate_ids, query_list, filter_criteria) {
    ...
    if (!is.null(query_list)) {
      keyword_ids <- map(names(query_list), function(scope) {
        target_df <- db[[scope]]
        search_col_name <- names(target_df)[3] # Simplification: assume 3rd col is desc
  ...
  # --- Assemble Final, INFORMATIVE Results ---
  if (length(final_ids) == 0) {
    message("\n>>> No tables found matching all specified criteria. <<<")
    return(tibble())
  }
  
  # Pre-aggregate metadata for efficient joining
  ...
  if (!is.null(semantic_candidates)) {
    results <- results %>%
      ...
      select(table_id, similarity, table_desc, period_raw, variables, classifications, geo_levels) # Select and order key columns
  } else {
    results <- results %>%
