...
# Esta é a função original, que agora será chamada pela função de cache
fetch_sidra_from_api <- function(tbl_config, muni_codes_vec){
  # O corpo desta função é o mesmo da sua fetch_sidra_data_helper original
  df <- tryCatch(
    sidrar::get_sidra(x = tbl_config$table, variable = (tbl_config$var_code %||% "all"), period = "all", geo = "City",
...
# Nova função "wrapper" que gerencia o cache
fetch_sidra_data_helper <- function(tbl_config, muni_codes_vec, cache_dir = "sidra_cache") {
  # 1. Cria o diretório de cache se ele não existir
  if (!dir.exists(cache_dir)) {
    dir.create(cache_dir, recursive = TRUE)
  }
  
  # 2. Cria uma chave única para a requisição, garantindo consistência
  #    A chave é baseada no ID da tabela e nos códigos de município ordenados
  request_key <- paste(tbl_config$table, paste(sort(muni_codes_vec), collapse = "-"), sep = "_")
  
  # 3. Gera um hash MD5 como nome de arquivo seguro
  cache_filename <- paste0(digest::digest(request_key, algo = "md5"), ".rds")
  cache_filepath <- file.path(cache_dir, cache_filename)
  
  # 4. Verifica se o arquivo de cache já existe
  if (file.exists(cache_filepath)) {
    # Se existe, lê o dado do cache em vez de chamar a API
    # message(paste0("CACHE HIT: Loading data for table ", tbl_config$table, " from ", cache_filepath))
    return(readRDS(cache_filepath))
  } else {
    # Se não existe, chama a função que busca os dados da API
    # message(paste0("CACHE MISS: Fetching data for table ", tbl_config$table, " from API."))
    fetched_data <- fetch_sidra_from_api(tbl_config, muni_codes_vec)
    
    # Salva o resultado no cache para uso futuro, somente se o fetch for bem-sucedido
    if (!is.null(fetched_data)) {
      saveRDS(fetched_data, file = cache_filepath)
    }
...
standardize_keys_types_helper <- function(df, table_id_for_log = "UnknownTable"){
  ...
  df_processed <- df %>%
    mutate(
      value = case_when(
        Variável %in% c("Salário médio mensal", "Salário médio mensal em reais") ~ readr::parse_number(Valor_raw, locale = readr::locale(decimal_mark = ".")),
        TRUE ~ readr::parse_number(Valor_raw, locale = readr::locale(decimal_mark = ","))
      ),
      value = if_else(Valor_raw %in% c("-", "...", "..", ""), NA_real_, value)
    )
  
  # THIS IS THE VALIDATED FIX: A safe select() that preserves classification columns.
  df_processed %>%
    rename(muni_code = `Município (Código)`, ano_chr = Ano) %>%
    mutate(
      muni_code = stringr::str_pad(as.character(muni_code), 7, "left", "0"),
      ano = as.integer(ano_chr),
      Variável = trimws(as.character(Variável))
    ) %>%
    select(
      muni_code, ano, Variável, value, Valor_raw,
      any_of(c("Fetched_Table_ID", "Fetched_Table_Desc")),
      everything(), # This preserves all other columns, including 'Grupo de idade'.
      -ano_chr, -Valor
    )
}
...
create_demographic_basis_pipeline <- function(
  ...
  # Preparar listas para armazenar dataframes processados
  processed_dfs <- list()
  
  # Processar tabela do censo 2000, se existir
  if (!is.null(std_data_list_pipeline$censo_2000) && nrow(std_data_list_pipeline$censo_2000) > 0) {
    processed_dfs$censo_2000 <- std_data_list_pipeline$censo_2000 %>%
      filter(ano == 2000) %>%
      rename(age_source_column = `Grupo de idade`) # Renomeia para um nome padrão
  }
  
  # Processar tabela dos censos 2010/22, se existir
  if (!is.null(std_data_list_pipeline$censo_10_22) && nrow(std_data_list_pipeline$censo_10_22) > 0) {
    processed_dfs$censo_10_22 <- std_data_list_pipeline$censo_10_22 %>%
      filter(ano %in% c(2010, 2022)) %>%
      rename(age_source_column = `Idade`) # Renomeia para o mesmo nome padrão
  }
  
  # Combinar os dataframes que agora possuem uma estrutura de coluna de idade consistente
  census_raw_combined <- bind_rows(processed_dfs)
  ...
  if (nrow(census_raw_combined) > 0) {
    ...
    # A lógica subsequente agora usa a coluna padronizada 'age_source_column'
    census_age_filt <- census_std %>%
      mutate(age_orig_raw = as.character(age_source_column)) %>%
    ...
    # O restante do passo 4 permanece inalterado
    census_harm <- census_age_filt %>%
      ...
      mutate(gender = recode(gender_orig, !!!SEX_CODES), race = recode(race_orig, !!!RACE_CODES), 
             age_band_target = purrr::map_chr(age_orig, harmonize_age_band_target_schema_scalar_helper)) %>%
  ...
  # --- Adicionado: Normalização dos dados granulares dos anos de censo ---
  # Garante que a soma das partes granulares seja exatamente igual ao total oficial do censo.
  if (exists("census_processed_for_pivot") && nrow(census_processed_for_pivot) > 0) {
    message("   Normalizing census-year granular data to match official totals...")
    census_processed_for_pivot <- census_processed_for_pivot %>%
      left_join(raw_census_totals, by = c("muni_code", "ano")) %>%
      # Para municípios onde o total é zero, a população granular também deve ser zero
      mutate(pop = ifelse(pop_total_original_raw == 0, 0, pop)) %>%
      group_by(muni_code, ano) %>%
      # Esta nova lógica é mais robusta e evita os avisos de vetorização do dplyr
      mutate(
        current_sum = sum(pop, na.rm = TRUE),
        diff = first(pop_total_original_raw) - current_sum
      ) %>%
      # A lógica de ajuste agora é feita fora do mutate principal para segurança
      group_modify(~ {
        .x_group <- .x
        diff_val <- first(.x_group$diff)
        
        if (is.na(diff_val) || diff_val == 0) {
          .x_group$pop_adj <- .x_group$pop
        } else {
          # Calcula o erro de arredondamento e distribui
          rem <- diff_val
          
          # Ordena por população decrescente para distribuir o erro nos maiores grupos primeiro
          indices_to_adjust <- order(.x_group$pop, decreasing = TRUE)
          
          # Adiciona ou subtrai 1 iterativamente
          for (i in 1:abs(rem)) {
            idx <- indices_to_adjust[i]
            .x_group$pop[idx] <- .x_group$pop[idx] + sign(rem)
          }
          .x_group$pop_adj <- .x_group$pop
        }
      ...
      ungroup() %>%
      select(-pop, -pop_total_original_raw, -current_sum, -diff) %>%
      rename(pop = pop_adj)
  }
  ...
  if(!is.null(all_states_geo)){
    state_info_df <- all_states_geo %>% sf::st_drop_geometry() %>% transmute(uf_code=as.character(code_state), uf_name=name_state, uf_abbr=abbrev_state)
    ...
    if(nrow(parsed_cols_for_interp_df) > 0 && all(sapply(parsed_cols_for_interp_df[,2:4], function(x) any(nzchar(x))))){
      ...
      interp_counts_integerized <- interp_counts_long_decimal %>%
        ...
        mutate(adj_val = ifelse(row_number() <= abs(diff_dist), sign(diff_dist), 0),
               pop_int_final = pmax(0, pop_r + adj_val)) %>% # <--- ADICIONE pmax() AQUI
